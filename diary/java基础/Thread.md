...
进程不共享内存空间，而线程共享相同的内存空间(同一个进程内的线程)
...
##用Runnable还是Thread？
Java没有多继承，所以Runnable接口可以帮助已经继承的Java类扩展线程的功能

##Thread 类中的start() 和 run() 方法有什么区别？
直接运行run()方法就是一个普通的方法调用
使用start()方法会新建一个thread,BUT...在JDK源代码中start()方法并没有显示的调用run()方法
```java
...
```
取而代之是调用了本地方法start0()，通过OpenJDK代码可知
JVM-> JVM_StartThread-> run_method_name -> 最终调用了目标线程的run()方法

##Java中Runnable和Callable有什么不同？
Callable的run()方法有返回值，它代表未来执行完成线程后的结果返回一个Future对象
Runnable的run()不返回任何值
...
volatile保证了一个变量的可见性，有序性，但是不保证原子性

##什么是线程安全？Vector是一个线程安全类吗？ （详见这里)
多个线程运行这段代码与单个线程运行这段代码结果一致，则线程安全
